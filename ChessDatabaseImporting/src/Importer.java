...
public class Importer {
	...
	private final int TOURNAMENTID = 1;
	...
	private final int MATCHHOSTID = 1;
	...
	/**
	 * Makes the default match host (FICS)
	 */
	...
	/**
	 * Makes the default judge (FICS.org)
	 */
	...
	/**
	 * Makes the default tournament (FICS Rated Standard Game)
	 */
	private void makeTournament() {
		CallableStatement cs;
		try {
			cs = c.prepareCall("{? = call TournamentInsertStartUp(?, ?, ?, ?, ?)}"); //The actual call.
	...
	private void makeCompetesIn(String player) {
		CallableStatement cs;
		//System.out.println(c);
		
		try {
			cs = c.prepareCall("{? = call CompetesInInsert(?, ?)}");
			cs.setString(2, player); 
			cs.setInt(3, TOURNAMENTID);
			cs.registerOutParameter(1, Types.INTEGER);
			cs.execute();
			if(cs.getInt(1) == 10) {
				System.out.println("Duplicate record not inserted: CompetesIn");
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
